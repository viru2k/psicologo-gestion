...
export class PopupControlEncabezadoComponent implements OnInit {
  ...
  nuevaUnidad() {
    this.loading = true;
    try {
      this.calidadService.setCalidadControlEncabezado(this.updateDataForm.value)
      .subscribe(resp => {
          this.loading = false;
          console.log(resp);
          this.ref.close(resp);
  ...
  editarUnidad() {
    ...
    try {
      this.calidadService.putCalidadControlEncabezado(  this.updateDataForm.value, this.updateDataForm.value.id)
      .subscribe(resp => {
        this.loading = false;
        console.log(resp);
        this.ref.close(resp);
